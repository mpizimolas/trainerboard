...
import { loadStore, saveStore, toggleAssignmentComplete, exerciseById } from "@/lib/store";

type SupaClientData = {
  client: { id: string; name: string; invite_token: string };
...
export default function ClientPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  useEffect(() => {
    if (!isSupaEnv) return;
    // Try Supabase fetch if localStorage client not found
    const localClient = store.clients.find((c) => c.invite_token === token);
    if (localClient) return; // demo mode takes precedence
    setSupaLoading(true);
    (async () => {
      try {
        const { createClient } = await import("@/lib/supabase/client");
        const supa = createClient();
        // Fetch client by token (requires anon SELECT policy - see supabase/schema.sql comments)
        const { data: client, error: e1 } = await supa.from("clients").select("id,name,invite_token").eq("invite_token", token).single();
  ...
  if (supaData) {
    ...
    return (
      <div className="min-h-screen bg-zinc-50 flex flex-col">
        <header className="bg-white border-b sticky top-0"><div className="max-w-lg mx-auto px-6 h-14 flex items-center justify-between"><span className="font-bold">TrainerBoard</span><span className="text-xs bg-green-600 text-white px-3 py-1 rounded-full">Supabase • {client.name}</span></div></header>
        <main className="flex-1 max-w-lg mx-auto w-full px-6 py-6 space-y-4">
          <div className="bg-white border rounded-2xl p-5"><h1 className="font-bold">Hi {client.name.split(" ")[0]} 👋</h1><p className="text-sm text-zinc-500">Trainer assigned workouts (live from Postgres).</p></div>
          ...
          {supaData.assignments.map((a) => (
            <div key={a.id} className="bg-white border rounded-2xl p-5">
              ...
              <button onClick={async()=>{ const { toggleAssignmentDb } = await import("@/lib/db"); await toggleAssignmentDb(a.id, a.status); location.reload(); }} className={`mt-3 text-xs px-3 py-1.5 rounded-full font-medium ${a.status==="completed" ? "bg-green-600 text-white" : "bg-zinc-900 text-white"}`}>{a.status==="completed" ? "Completed ✓" : "Mark complete"}</button>
            </div>
          ))}
  ...
  return (
    <div className="min-h-screen bg-zinc-50 flex flex-col">
      ...
      <main className="flex-1 max-w-lg mx-auto w-full px-6 py-6 space-y-4">
        <div className="bg-white border rounded-2xl p-5"><h1 className="font-bold text-lg">Hi {client.name.split(" ")[0]} 👋</h1><p className="text-sm text-zinc-500">Your trainer assigned workouts below. Check off sets and your trainer sees it instantly.</p><p className="text-xs text-zinc-400 mt-2">PWA: Add to Home Screen for app-like experience.</p></div>
        ...
        {assignments.map((a) => {
          ...
          return (
            <div key={a.id} className="bg-white border rounded-2xl p-5">
              <div className="flex justify-between items-start"><div><div className="font-semibold">{w.title}</div><div className="text-xs text-zinc-500">{a.assigned_date} • {a.status} • {w.notes}</div></div><button onClick={() => setStore((s)=> toggleAssignmentComplete(s, a.id))} className={`text-xs px-3 py-1.5 rounded-full font-medium ${a.status==="completed" ? "bg-green-600 text-white" : "bg-zinc-900 text-white"}`}>{a.status==="completed" ? "Completed ✓" : "Mark complete"}</button></div>
