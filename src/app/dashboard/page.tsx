...
import { loadStore, saveStore, createClient as createClientLocal, createWorkout as createWorkoutLocal, assignWorkout as assignWorkoutLocal, toggleAssignmentComplete, exerciseById, getExercises } from "@/lib/store";

const isSupaEnv = typeof process !== "undefined" && Boolean(process.env.NEXT_PUBLIC_SUPABASE_URL);

type SupaState = {
  user: { id: string; email?: string } | null;
  clients: Array<{ id: string; name: string; email: string | null; phone: string | null; invite_token: string; status: string }>;
...
export default function DashboardPage() {
  ...
  const clients = useSupa ? supa!.clients : store.clients;
  const workouts = useSupa ? supa!.workouts.map(w => ({ id: w.id, title: w.title, notes: w.notes ?? "", exercises: (w.workout_exercises ?? []).map((we: { exercise_id: string; sets: number; reps: string; rest_seconds: number }) => ({ exercise_id: we.exercise_id, sets: we.sets, reps: we.reps, rest: we.rest_seconds })) })) : store.workouts;
  ...
  // Handlers that branch
  async function handleAddClient() {
    if (!name) return alert("Name required");
    if (useSupa) {
      try { const { createClientDb } = await import("@/lib/db"); await createClientDb(name, email, phone); await loadSupa(); setName(""); setEmail(""); setPhone(""); } catch(e: unknown){ alert(e instanceof Error ? e.message : String(e)); }
    } else {
      setStore((s) => createClientLocal(s, { name, email, phone })); setName(""); setEmail(""); setPhone("");
    }
  }
  ...
  async function handleAssign() {
    ...
    if (useSupa) {
      try { const { assignWorkoutDb } = await import("@/lib/db"); await assignWorkoutDb(assignWorkoutId, assignClientId, assignDate); await loadSupa(); } catch(e: unknown){ alert(e instanceof Error ? e.message : String(e)); }
    } else {
      setStore((s) => assignWorkoutLocal(s, assignWorkoutId, assignClientId, assignDate));
    }
  }
  async function handleToggle(aId: string, status: string) {
    if (useSupa) {
      const { toggleAssignmentDb } = await import("@/lib/db"); await toggleAssignmentDb(aId, status); await loadSupa();
    } else {
      setStore((s) => toggleAssignmentComplete(s, aId));
    }
  }
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white border rounded-3xl p-6 shadow-sm">
            ...
            <div className="mt-4 space-y-3">
              ...
              {todaysAssignments.map((a) => {
                ...
                const c = clients.find((x) => x.id === a.client_id) as { invite_token?: string; name?: string } | undefined;
                return (
                  <div key={a.id} className="flex items-center justify-between border rounded-xl px-4 py-3">
                    <div>
                      <div className="font-medium text-sm">{w?.title} → {(c as {name?:string})?.name}</div>
                      <div className="text-xs text-zinc-500">{a.status} • <Link href={`/c/${(c as {invite_token?:string})?.invite_token ?? ""}`} className="underline">Client link</Link></div>
                    </div>
                    <button onClick={() => handleToggle(a.id, a.status)} className={`text-xs px-3 py-1.5 rounded-full font-medium ${a.status === "completed" ? "bg-green-600 text-white" : "bg-zinc-900 text-white"}`}>{a.status === "completed" ? "Completed ✓" : "Mark done"}</button>
                  </div>
                );
              })}
            </div>
          ...
          <div className="bg-white border rounded-2xl p-5">
            <h2 className="font-semibold">Clients ({clients.length}{useSupa ? "" : "/3 free"})</h2>
            <div className="mt-3 space-y-2">
              {clients.map((c) => (
                <div key={c.id} className="flex items-center justify-between border rounded-xl px-4 py-3 text-sm">
                  <div><div className="font-medium">{(c as {name:string}).name}</div><div className="text-xs text-zinc-500">{(c as {email:string}).email} • {(c as {phone:string}).phone} • {(c as {status:string}).status}</div></div>
                  <Link href={`/c/${(c as {invite_token:string}).invite_token}`} className="text-xs border px-3 py-1.5 rounded-full hover:bg-zinc-50">Open client link</Link>
                </div>
              ))}
            </div>
            <div className="mt-5 border-t pt-4">
              <h3 className="text-sm font-semibold">Add client (10s)</h3>
              <div className="mt-3 grid sm:grid-cols-3 gap-2">
                <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Name" className="border rounded-xl px-3 py-2 text-sm" />
                <input value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Email" className="border rounded-xl px-3 py-2 text-sm" />
                <input value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="Phone" className="border rounded-xl px-3 py-2 text-sm" />
              </div>
              <button onClick={handleAddClient} className="mt-3 bg-zinc-900 text-white px-5 py-2 rounded-full text-sm font-medium">Add client</button>
              {!useSupa && clients.length >= 3 && <p className="text-xs text-amber-600 mt-2">Free limit 3 — $19 Starter for 15.</p>}
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="bg-white border rounded-2xl p-5">
            <h2 className="font-semibold">Workouts ({workouts.length})</h2>
            <div className="mt-3 space-y-2 max-h-64 overflow-auto pr-1">
              {workouts.map((w) => (
                <div key={w.id} className="border rounded-xl p-3">
                  <div className="font-medium text-sm">{w.title}</div>
                  <div className="text-xs text-zinc-500">{(w as {exercises: Array<{exercise_id:string;sets:number;reps:string;rest:number}>}).exercises.length} exercises • {w.notes}</div>
            ...
            <div className="mt-5 border-t pt-4">
              <h3 className="text-sm font-semibold">New workout template</h3>
              <input value={wTitle} onChange={(e) => setWTitle(e.target.value)} placeholder="Title e.g. Push Day" className="mt-2 w-full border rounded-xl px-3 py-2 text-sm" />
              <input value={wNotes} onChange={(e) => setWNotes(e.target.value)} placeholder="Notes" className="mt-2 w-full border rounded-xl px-3 py-2 text-sm" />
              {wExercises.map((ex, idx) => (
                <div key={idx} className="mt-2 flex gap-1 items-center">
                  <select value={ex.exercise_id} onChange={(e) => setWExercises((arr) => arr.map((x, i) => i === idx ? { ...x, exercise_id: e.target.value } : x))} className="flex-1 border rounded-xl px-2 py-2 text-xs">
              ...
              <div className="mt-2 flex gap-2">
                <button onClick={() => setWExercises((a) => [...a, { exercise_id: "3", sets: 3, reps: "10", rest: 60 }])} className="text-xs border px-3 py-1.5 rounded-full">+ Add exercise</button>
                <button onClick={handleSaveWorkout} className="text-xs bg-zinc-900 text-white px-4 py-1.5 rounded-full">Save template</button>
              </div>
            </div>
          </div>
          ...
          <div className="bg-zinc-900 text-white rounded-3xl p-6 shadow-lg relative overflow-hidden">
            ...
            <select value={assignClientId} onChange={(e) => setAssignClientId(e.target.value)} className="mt-2 w-full rounded-xl px-3 py-2 text-sm text-zinc-900 bg-white">
              {clients.map((c) => <option key={c.id} value={c.id}>{(c as {name:string}).name}</option>)}
            </select>
            <input type="date" value={assignDate} onChange={(e) => setAssignDate(e.target.value)} className="mt-2 w-full rounded-xl px-3 py-2 text-sm text-zinc-900 bg-white" />
            <button onClick={handleAssign} className="mt-3 w-full bg-white text-zinc-900 py-2 rounded-full text-sm font-medium">Assign →</button>
            <p className="text-xs text-zinc-400 mt-2">Client gets magic link <span className="text-zinc-200">/c/[token]</span> — add as PWA.</p>
          </div>
        </div>
      </div>
    </div>
